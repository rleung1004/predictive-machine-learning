...
with tf.device('/cpu:0'):
    def grid_search(random: bool = False, filename: str = "grid_search"):
        ...
        y = df.copy(True)
        y = y[['current price']]
        x = pd.get_dummies(df, columns=['condition'])
        x = x.drop(['economy', 'rating', 'current price', 'top speed', 'hp', 'torque', 'condition_1', 'condition_2',
                     'condition_3'], axis=1)
        print(x.head())
    ...
    if __name__ == '__main__':
        grid_search(random=True, filename="test2")
